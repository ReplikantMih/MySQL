...
(SELECT freelancers.id AS id, balance 
	FROM accounts 
	RIGHT JOIN freelancers ON accounts.id = freelancers.account_id 
	ORDER BY balance DESC 
	LIMIT 10)
		UNION ALL
(SELECT clients.id, balance 
	FROM accounts 
	RIGHT JOIN clients ON accounts.id = clients.account_id 
	ORDER BY balance DESC 
	LIMIT 10) 
		ORDER BY balance DESC;
	...
	WHERE projects.id IN  
		(SELECT projects.id	
			FROM (SELECT * FROM projects) AS copy -- MySQL не поддерживает конструкции где UPDATE производится над таблицей,
				RIGHT JOIN project_statuses 	  -- которая фигурирует напрямую в условии WHERE, поэтому такой дополнительный вложенный запрос.
